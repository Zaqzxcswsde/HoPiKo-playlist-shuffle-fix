...
1. Grab the [patched binary](https://github.com/Zaqzxcswsde/HoPiKo-playlist-shuffle-fix/releases/download/v1.0.0/Assembly-CSharp.dll) from the latest release
...
Verify original `Assembly-CSharp.dll` hash:
    ...
    1. You should see this value in the output:
        ...
        - if the hash differs, this means that the game has been updated and you would need to patch the game manually, read about it below
...
This mod is incompatible with other mods that use the `Assembly-CSharp.dll` patching method.
...
If you would rather apply the patch manually (or if the original patched binary no longer works), here's how you can do that:
...
1. In the newly opened window, replace all (`Ctrl+A`) code with the contents of `Update.cs` file from this repository, click `Compile`
...
1. After the build finishes, you can close `dnSpy`, your game is successfully patched
...
- To make your life easier, I've included the condensed write-up of the research that AI performed in the `docs` folder, so you wouldn't have to start from scratch
